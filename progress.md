...
**최종 업데이트:** 2026-04-18 (v2 Architecture Reset — Soft-launch 배포 완료)
...
## Progress
- **Soft-launch 배포 완료 (2026-04-18). 프로덕션 URL: https://terrasight.pages.dev (CF Pages) + https://terrasight-worker.myongpunch08.workers.dev (CF Workers). 152-file uncommitted → 7 logical commits (legacy quarantine / pipelines / monorepo / worker / web / data / docs), working tree clean. Worker 배포 (23.87 KiB gzipped, Startup 19 ms) → 3 routes live (`/health`·`/api/earthquakes`·`/api/fires`·`/api/sst-point`) w/ CORS accepting Pages origin. Web prod build (16 pages, 15.37s) with `PUBLIC_SITE_URL=https://terrasight.pages.dev` + `PUBLIC_WORKER_BASE_URL=https://terrasight-worker.myongpunch08.workers.dev` → wrangler pages deploy (27 files, 3.65s upload). Smoke matrix post-deploy: / (200, canonical emits) · /globe/ (200) · /reports/{la,nyc,houston}/ (200) · /rankings/{4 metrics}/ (200) · sitemap-index.xml (200) · robots.txt (200, 4 guide-stub Disallow lines). Home canonical P2 from Step 10 QA closed (1-line `seo` prop wired before deploy). Git-connected Pages project — direct upload bypasses git pipeline OK. Next: attach custom domain post-preview signoff + AdSense runtime wiring (Step 9+) + Render stub retirement after 2-week Worker-only traffic window.**
